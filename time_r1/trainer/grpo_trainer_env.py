...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.environment is not None:
            ...
            with profiling_context(self, "environment.generate"):
                ...
                generated_messages = self.environment.generate(gathered_messages, multimodal_cache, 
                                                               sampling_params=sampling_params, profiling_metrics=env_profiling_metrics)
                # log to wandb
            ...
            if self.args.use_counterfactual_reasoning:
                ...
                with profiling_context(self, "environment.counterfactual_reasoning"):
                    visual_trace_batch, has_visual_trace_batch = get_visual_trace(generated_messages, questions)
                    visual_trace_batch_filtered = [visual_trace_batch[i] for i in range(len(visual_trace_batch)) if has_visual_trace_batch[i]]
                    visual_trace_batch_prediction_texts = ["I don't know"] * len(visual_trace_batch)
                    if len(visual_trace_batch_filtered) > 0:
                        valid_visual_trace_batch_prediction_texts = self.environment.single_turn_generate(visual_trace_batch_filtered, sampling_params=sampling_params)
                    # 根据has_visual_trace_batch，将valid_visual_trace_batch_prediction_texts填充到visual_trace_batch_prediction_texts中
                    current_idx = 0
                    for i in range(len(has_visual_trace_batch)):
                        if has_visual_trace_batch[i]:
                            visual_trace_batch_prediction_texts[i] = valid_visual_trace_batch_prediction_texts[current_idx]
                            current_idx += 1
                    # print(f"visual_trace_batch_prediction_texts: {visual_trace_batch_prediction_texts}")
            if self.vllm_tensor_parallel_size > 1:
                # Slice completions for this rank within its TP group.
                # Each rank generates all outputs — we keep only our share.
                local_rank_in_group = torch.distributed.get_rank(group=self.tp_group)
                tp_slice = slice(local_rank_in_group * orig_size, (local_rank_in_group + 1) * orig_size)
                generated_messages = [generated_messages[i] for i in tp_slice]
            generated_texts = self.processing_class.apply_chat_template(generated_messages, tokenize=False, add_generation_prompt=False)
            ...
            prompt_completion_inputs = self.processing_class(
                ...
                padding=True,
                return_tensors="pt",
                padding_side="right",   # NOTE: padding_side is right, because we want to keep the model response tokens
                add_special_tokens=False,
            )   # shape: (1, N)
            # Move to device
            prompt_completion_inputs = super()._prepare_inputs(prompt_completion_inputs)
            # Don't forget to update multimodal_inputs
            multimodal_inputs = {
                k: v for k, v in prompt_completion_inputs.items() if k not in ["input_ids", "attention_mask"]
            }
            prompt_completion_ids = prompt_completion_inputs["input_ids"]
            prompt_completion_mask = prompt_completion_inputs["attention_mask"]
            # ------------------------------
            prompt_length = prompt_inputs["input_ids"].size(1)
            left_paddings = (prompt_inputs["attention_mask"] == 0).sum(dim=1)   # left padding size, before max_prompt_length truncation
            prompt_actual_lengths = prompt_length - left_paddings  # actual content length of each sample
            completion_ids, completion_mask = extract_completion_from_full_sequence(
            ...
            prompt_ids_new = pad(prompt_ids_no_padding, padding_value=self.processing_class.pad_token_id, padding_side="left")
            prompt_attention_mask_new = pad(prompt_attention_mask_no_padding, padding_value=0, padding_side="left")
            # assert torch.all(prompt_ids_new == prompt_ids), f"prompt_ids_new: {prompt_ids_new.shape}, prompt_ids: {prompt_ids.shape}"
            ...
            completion_mask = compute_tool_response_mask(completion_ids) * completion_mask
            ...
            if self.vllm_mode == "server":
                raise NotImplementedError("I don't want to handle vLLM server mode in VL-GRPO")
        ...
        if self.environment is None:
            # NOTE: Do NOT apply single-turn completion mask to multi-turn completions
        ...
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            ...
            with profiling_context(self, reward_func_name):
                if isinstance(
                    ...
                    keys = [key for key in inputs[0] if key not in ["prompt", "completion", "messages", "multimodal_cache"]]
                    reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
                    if self.environment is None:
                        output_reward_func = reward_func(prompts=prompts, completions=completions_text, **reward_kwargs)
                    elif self.args.use_counterfactual_reasoning:
                        # 反事实推理
                        output_reward_func = reward_func(prompts=prompts, completions=completions_text, messages=generated_messages, visual_trace_completions=visual_trace_batch_prediction_texts, **reward_kwargs)
                    else:
...
def extract_completion_from_full_sequence(
    ...
    for i in range(batch_size):
        ...
        valid_indices = curr_completion_mask.nonzero(as_tuple=True)[0]
...
def get_visual_trace(batch_messages, batch_questions):
    """
    ...
    for messages, question in zip(batch_messages, batch_questions):
        filtered_messages = []
        filtered_messages.append({
            "role": "system", 
            "content": f"You are a helpful assistant. Please answer visual questions as briefly as possible. When you don't have enough visual information, please say 'I don't know'."
        })
        content = []
